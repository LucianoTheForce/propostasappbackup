...
function InteractiveLogo() {
  ...
  // Carrega o modelo GLB com error handling
  const { scene, materials } = useGLTF("/forcelogo.glb")
  ...
  // Aplicar materiais avançados
  useEffect(() => {
    if (materials) {
      Object.values(materials).forEach((material) => {
        if (material && typeof material === 'object' && 'isMeshStandardMaterial' in material) {
          const stdMaterial = material as THREE.MeshStandardMaterial
          stdMaterial.metalness = 0.8
          stdMaterial.roughness = 0.2
          stdMaterial.clearcoat = 1.0
          stdMaterial.clearcoatRoughness = 0.1
          stdMaterial.envMapIntensity = 1.5
        }
      })
    }
  }, [materials])
  ...
  // Sistema de eventos de interação igual ao código de referência
  useEffect(() => {
    const handleInteractionStart = (e: MouseEvent | TouchEvent) => {
      // Para touch, verificar se está na área do canvas/modelo
      if ("touches" in e && e.touches.length > 0) {
        const touch = e.touches[0]
        const target = e.target as Element
        
        // Só iniciar interação se estiver na área do canvas 3D
        if (!target.closest('canvas') && !target.closest('[data-three-canvas]')) {
    ...
    const handleInteractionMove = (e: MouseEvent | TouchEvent) => {
      ...
      // Prevenir scroll no mobile apenas durante interação ativa
      if ("touches" in e) {
        e.preventDefault()
      }
  ...
  useFrame((state, delta) => {
    ...
    // Debug: log da respiração a cada 2 segundos
    if (Math.floor(currentTime * 0.5) % 2 === 0 && Math.floor(currentTime * 10) % 10 === 0) {
      console.log('Breathing scale:', breathScale, 'Final scale will be:', targetScale * 3)
    }

    // Calcular escala responsiva - MUITO maior agora
    const viewportSize = typeof window !== "undefined" ? Math.min(window.innerWidth, window.innerHeight) : 800
    const baseScale = viewportSize * 0.025 // Aumentado drasticamente para ser 3x maior
    const responsiveScale = Math.max(baseScale, 20) // Base de 20x para ficar bem grande
  ...
  return (
    <group>
      <group ref={groupRef} position={[0, 0, 0]}>
        ...
        <group ref={ringRef}>
          {/* Adicionar elementos do anel aqui se necessário */}
        </group>
      </group>
    </group>
  )
...
function Lighting() {
  return (
    <>
      ...
      {/* Ambiente HDRI para reflexões realistas */}
...
export function InteractiveLogoHero({ className = "" }: InteractiveLogoHeroProps) {
  ...
  return (
    <div className={className} style={{ touchAction: 'none', cursor: 'grab' }} data-three-canvas>
      <Canvas
